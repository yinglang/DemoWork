...
void UILayer::saveEvent(Ref* psender, ui::Widget::TouchEventType type){
	if (type == ui::Widget::TouchEventType::ENDED){
		...
		if (_desinger) _desinger->saveCompleteDataToXML(_desinger->defaultSaveFilepath);
	}
}
...
void Box2DDesignerScene::addEdge(Vec2 startPoint, Vec2 endPoint){
	...
	bodydata->name = "body" + num2str(world->GetBodyCount()-1);
...
void Box2DDesignerScene::saveCompleteDataToXML(std::string filepath){
	if (filepath == "") filepath = FileUtils::getInstance()->getWritablePath() + "bos2ddesignerComplete.xml";
	saving = true;
